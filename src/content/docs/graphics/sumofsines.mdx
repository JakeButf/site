...
<img src="https://latex.codecogs.com/svg.image?\fg{white}&space;H(x,y,t)=\sum(A_i*sin(D_i)\bullet(x,y)*w_i&plus;t*\varphi))" title=" H(x,y,t)=\sum(A_i*sin(D_i)\bullet(x,y)*w_i+t*\varphi))" />
Where:
```
A = Amplitude
